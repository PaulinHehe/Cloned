...
async function fetchBlameAllBranches(owner, repo, local, commits) {
  try {
    ...
    // Créer un ensemble de dates où des commits ont été publiés
    ...
    allCommits.forEach(commit => {
      try {
        ...
        console.error("❌ Error processing date:", commit.author.date, error.message);
      }
    });
    ...
    const start = parseISO(sinceDate); // sinceDate doit être défini dans votre contexte global
    const end = parseISO(untilDate);   // untilDate doit être défini dans votre contexte global


    for (let d = start; !isAfter(d, end); d = addDays(d, 1)) {
      const dateStr = format(d, 'yyyy-MM-dd');

      // Ne traiter que les jours où il y a eu au moins un commit
      if (!commitDates.has(dateStr)) {
        //console.log(`⏩ Pas de commits pour le ${dateStr}, on passe.`);
        continue;
      }
...
async function fetchAndProcessCommits(owner, repo, local, totalCommits) {
  try {
    // Supposons que fetchCommits est une fonction asynchrone qui récupère les commits
